...
command_timer: ?std.Io.Clock.Timestamp = null,

/// Search state
search: ?Search = null,

/// Used to rate limit BEL handling.
last_bell_time: ?std.Io.Clock.Timestamp = null,
...
pub fn init(
    ...
    self.* = .{
        .id = id: {
            while (true) {
                ...
                std.Io.random(global_state.io, &buf);
                const candidate = std.mem.readInt(u64, &buf, .little);
    ...
    self.renderer_thr.setName(global_state.io, "renderer") catch {};
    ...
    self.io_thr.setName(global_state.io, "io") catch {};
    ...
    switch (msg) {
        ...
        .ring_bell => bell: {
            ...
            if (self.last_bell_time) |last| {
                if (last.durationTo(now).raw.nanoseconds < 100 * std.time.ns_per_ms) break :bell;
            }
        ...
        .start_command => {
            self.command_timer = .now(global_state.io, .awake);
        },

        .stop_command => |v| timer: {
            const end: std.Io.Clock.Timestamp = .now(global_state.io, .awake);
            const start = self.command_timer orelse break :timer;
            self.command_timer = null;

            const duration: Duration = .{
                .duration = @intCast(start.durationTo(end).raw.nanoseconds),
            };
...
fn resolvePathForOpening(
    ...
    if (!std.fs.path.isAbsolute(path)) {
        ...
        std.Io.Dir.accessAbsolute(global_state.io, resolved, .{}) catch {
...
fn mouseShiftCapture(self: *const Surface, lock: bool) bool {
    ...
    if (lock) self.renderer_state.mutex.lockUncancelable(global_state.io);
    defer if (lock) self.renderer_state.mutex.unlock(global_state.io);
...
pub fn mouseButtonCallback(
    ...
    if (button == .left and action == .press) {
        ...
        extend_selection: {
            ...
            // an increment then we do not extend the selection.
            {
                ...
                const since: u64 = @intCast(click_time.durationTo(now).raw.nanoseconds);
    ...
    if (button == .left and action == .press) click: {
        ...
        const time: std.Io.Clock.Timestamp = .now(global_state.io, .awake);
    ...
    if (button == .right and action == .press) sel: {
        ...
        switch (self.config.right_click_action) {
            ...
            .@"copy-or-paste" => if (self.io.terminal.screens.active.selection) |sel| {
                ...
                self.renderer_state.mutex.unlock(global_state.io);
                defer self.renderer_state.mutex.lockUncancelable(global_state.io);
    ...
    switch (action.scoped(.surface).?) {
        ...
        .search => |text| search: {
            const s: *Search = if (self.search) |*s| s else init: {
                ...
                s.thread.setName(global_state.io, "search") catch {};
...
fn writeScreenFile(
    ...
    const path_len = try tmp_dir.dir.realPathFile(global_state.io, filename, &path_buf);
...
fn showDesktopNotification(self: *Surface, title: [:0]const u8, body: [:0]const u8) !void {
    ...
    if (self.app.last_notification_time) |last| {
        if (last.durationTo(now).raw.nanoseconds < 1 * std.time.ns_per_s) {
    ...
    if (self.app.last_notification_time) |last| {
        if (self.app.last_notification_digest == new_digest) {
            if (last.durationTo(now).raw.nanoseconds < 5 * std.time.ns_per_s) {

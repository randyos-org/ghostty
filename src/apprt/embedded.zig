...
pub const Platform = union(PlatformTag) {
    ...
    pub fn init(tag_int: c_int, c_platform: C) !Platform {
        const tag = try (std.enums.fromInt(PlatformTag, tag_int) orelse error.InvalidEnumTag);
        return switch (tag) {
...
pub const CAPI = struct {
    ...
    export fn ghostty_app_set_color_scheme(v: *App, scheme_raw: c_int) void {
        const scheme = (std.enums.fromInt(apprt.ColorScheme, scheme_raw) orelse error.InvalidEnumTag) catch {
            log.warn(
    ...
    export fn ghostty_surface_mouse_pressure(
        ...
        const stage = (std.enums.fromInt(
            ...
            stage_raw,
        ) orelse error.InvalidEnumTag) catch {
            log.warn(

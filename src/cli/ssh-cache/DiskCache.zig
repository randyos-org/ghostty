...
pub fn clear(self: DiskCache) !void {
    std.Io.Dir.deleteFileAbsolute(global_state.io, self.path) catch |err| switch (err) {
        error.FileNotFound => {},
        else => return err,
    };
}
...
pub fn add(
    ...
    const file = std.Io.Dir.createFileAbsolute(global_state.io, self.path, .{
        .read = true,
        .truncate = false,
        .permissions = switch (builtin.os.tag) {
        ...
        error.PathAlreadyExists => blk: {
            ...
            errdefer existing_file.close(global_state.io);
    ...
    if (comptime builtin.os.tag != .windows) _ = file.tryLock(global_state.io, .exclusive) catch return error.CacheLocked;
    defer if (comptime builtin.os.tag != .windows) file.unlock(global_state.io);
...
fn fixupPermissions(file: std.Io.File) !void {
    ...
    if (@intFromEnum(stat.permissions) & 0o777 != 0o600) {
        try file.setPermissions(global_state.io, .fromMode(0o600));
    }
}
...
fn writeCacheFile(
    ...
    var dir = try std.Io.Dir.openDirAbsolute(global_state.io, cache_dir, .{});
    defer dir.close(global_state.io);

    var atomic_file = try dir.createFileAtomic(global_state.io, cache_basename, .{
        .permissions = switch (builtin.os.tag) {
            .windows => .default_file,
            else => .fromMode(0o600),
        },
    ...
    defer atomic_file.deinit(global_state.io);

    var buf: [1024]u8 = undefined;
    var file_writer = atomic_file.file.writer(global_state.io, &buf);
    var iter = entries.iterator();
    while (iter.next()) |kv| {
    ...
    try atomic_file.replace(global_state.io);
}
...
fn readEntries(
    ...
    var reader = file.reader(global_state.io, &.{});
...
test "disk cache operations" {
    ...
    const cache: DiskCache = .{ .path = path };
    try cache.add(alloc, "example.com", internal_os.unixTimestamp(testing.io));
    try cache.add(alloc, "example.com", internal_os.unixTimestamp(testing.io));
...
test "disk cache cleans up temp files" {
    ...
    try cache.add(alloc, "example.com", internal_os.unixTimestamp(testing.io));
    try cache.add(alloc, "example.org", internal_os.unixTimestamp(testing.io));

...
pub fn openPath(alloc_gpa: Allocator) ![:0]const u8 {
    ...
    if (std.fs.path.dirname(config_path)) |config_dir| {
        try std.Io.Dir.createDirPath(.cwd(), global_state.io, config_dir);
    }
...
fn configPath(alloc_arena: Allocator) ![]const u8 {
    ...
    for (paths) |path| {
        const f = std.Io.Dir.openFileAbsolute(global_state.io, path, .{}) catch |err| {
            switch (err) {
        ...
        const stat = try f.stat(global_state.io);

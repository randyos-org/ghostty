...
pub fn open(
    ...
    if (std.fs.path.isAbsolute(theme)) {
        ...
        const stat = file.stat(global_state.io) catch |err| {
    ...
    while (try it.next()) |loc| {
        ...
        if (cwd.openFile(global_state.io, path, .{})) |file| {
            const stat = file.stat(global_state.io) catch |err| {
                try diags.append(arena_alloc, .{
                    .message = try std.fmt.allocPrintSentinel(
                        arena_alloc,
...
pub fn openAbsolute(
    ...
    return std.Io.Dir.openFileAbsolute(global_state.io, theme, .{}) catch |err| {
        switch (err) {

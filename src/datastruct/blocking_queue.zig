...
pub fn BlockingQueue(
    ...
    return struct {
            ...
            if (self.full()) {
                switch (timeout) {
                    ...
                    .ns => |ns| {
                        self.not_full_waiters += 1;
                        defer self.not_full_waiters -= 1;
                        self.cond_not_full.waitTimeout(io, &self.mutex, .{
                            .duration = .{ .raw = .fromNanoseconds(ns), .clock = .awake },
                        }) catch return 0;
                    },
                }
        ...
        pub fn drain(self: *Self) DrainIterator {
            self.mutex.lockUncancelable(global_state.io);
            return .{ .queue = self };
        }
        ...
        pub const DrainIterator = struct {
            ...
            pub fn deinit(self: *DrainIterator) void {
                // If we have consumers waiting on a full queue, notify.
                if (self.queue.not_full_waiters > 0) self.queue.cond_not_full.signal(global_state.io);

                // Unlock
                self.queue.mutex.unlock(global_state.io);
            }
        };

...
pub fn getIndex(
    ...
    {
        self.lock.lockSharedUncancelable(global_state.io);
        defer self.lock.unlockShared(global_state.io);
        if (self.codepoints.get(key)) |v| return v;
    }

    // Slow path: we need to search this codepoint
    self.lock.lockUncancelable(global_state.io);
    defer self.lock.unlock(global_state.io);

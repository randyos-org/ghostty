...
pub const Windows = struct {
    ...
    pub const DiscoverIterator = struct {
        ...
        pub fn deinit(self: *DiscoverIterator) void {
            if (self.dir) |*d| d.close(global_state.io);
        ...
        pub fn next(self: *DiscoverIterator) !?DeferredFace {
            while (true) {
                ...
                const entry = (self.iter.?.next(global_state.io) catch null) orelse {
        ...
        fn systemFontsPath(self: *DiscoverIterator) ?[:0]const u8 {
            const systemroot = std.process.Environ.getAlloc(
                .{ .block = .global },
                self.alloc,
                "SYSTEMROOT",
            ) catch return null;
        ...
        fn userFontsPath(self: *DiscoverIterator) ?[:0]const u8 {
            const local_appdata = std.process.Environ.getAlloc(
                .{ .block = .global },
                self.alloc,
                "LOCALAPPDATA",
            ) catch return null;

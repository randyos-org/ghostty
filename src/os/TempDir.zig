...
pub fn init() !TempDir {
    ...
    const dir = dir: {
        ...
        break :dir try cwd.openDir(global_state.io, tmp_dir, .{});
    };
    ...
    while (true) {
        ...
        dir.createDir(global_state.io, tmp_path, .default_dir) catch |err| switch (err) {
            error.PathAlreadyExists => continue,
            else => |e| return e,
        };

        return TempDir{
            .dir = try dir.openDir(global_state.io, tmp_path, .{}),
            .parent = dir,
...
pub fn deinit(self: *TempDir) void {
    self.dir.close(global_state.io);
    self.parent.deleteTree(global_state.io, self.name()) catch |err|
...
test {
    ...
    var dir = try td.parent.openDir(global_state.io, nameval, .{});
    dir.close(global_state.io);

    // Should be deleted after we deinit
    td.deinit();
    try testing.expectError(error.FileNotFound, td.parent.openDir(global_state.io, nameval, .{}));
}

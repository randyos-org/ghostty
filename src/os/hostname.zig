...
pub fn isLocal(hostname: []const u8) LocalHostnameValidationError!bool {
    ...
    switch (builtin.os.tag) {
        .windows => {
            ...
            if (windows.exp.kernel32.GetComputerNameA(&buf, &nSize) == .FALSE) return false;
...
test "isLocal returns true when hostname is local" {
    switch (builtin.os.tag) {
        .windows => {
            ...
            var buf: [256:0]u8 = undefined;
            var nSize: windows.DWORD = buf.len;
            if (windows.exp.kernel32.GetComputerNameA(&buf, &nSize) == .FALSE) return error.GetComputerNameFailed;

...
pub fn Renderer(comptime GraphicsAPI: type) type {
    return struct {
        ...
        first_frame_time: ?std.Io.Clock.Timestamp = null,
        ...
        const SwapChain = struct {
            ...
            pub fn deinit(self: *SwapChain) void {
                ...
                for (0..buf_count) |_| self.frame_sema.waitUncancelable(global_state.io);
            ...
            pub fn nextFrame(self: *SwapChain) error{Defunct}!*FrameState {
                ...
                self.frame_sema.waitUncancelable(global_state.io);
                errdefer self.frame_sema.post(global_state.io);
            ...
            pub fn releaseFrame(self: *SwapChain) void {
                self.frame_sema.post(global_state.io);
            }
        };
        ...
        pub fn displayRealized(self: *Self) !void {
            ...
            self.draw_mutex.lockUncancelable(global_state.io);
            defer self.draw_mutex.unlock(global_state.io);
        ...
        pub fn updateFrame(
            ...
            var critical: Critical = critical: {
                ...
                state.mutex.lockUncancelable(global_state.io);
                defer state.mutex.unlock(global_state.io);
        ...
        pub fn drawFrame(
            ...
            texture: {
                ...
                self.font_grid.lock.lockSharedUncancelable(global_state.io);
                defer self.font_grid.lock.unlockShared(global_state.io);
        ...
        fn prepBackgroundImage(self: *Self) !void {
            ...
            if (self.config.bg_image) |p| load_background: {
                ...
                var file_reader = file.reader(global_state.io, &read_buf);
                const contents = file_reader.interface.allocRemaining(
                    self.alloc,
                    .limited(std.math.maxInt(u32)), // Max size of 4 GiB, for now.
                ) catch |err| {
                    log.warn(
        ...
        fn updateCustomShaderUniformsForFrame(self: *Self) !void {
            ...
            const since_ns: f32 = @floatFromInt(first_frame_time.durationTo(now).raw.nanoseconds);
            uniforms.time = since_ns / std.time.ns_per_s;

            const delta_ns: f32 = @floatFromInt(last_frame_time.durationTo(now).raw.nanoseconds);

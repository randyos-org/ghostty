...
pub fn set_action(event_: Event, action: key.Action) callconv(lib.calling_conv) void {
    if (comptime std.debug.runtime_safety) {
        _ = (std.enums.fromInt(key.Action, @intFromEnum(action)) orelse error.InvalidEnumTag) catch {
...
pub fn set_key(event_: Event, k: key.Key) callconv(lib.calling_conv) void {
    if (comptime std.debug.runtime_safety) {
        _ = (std.enums.fromInt(key.Key, @intFromEnum(k)) orelse error.InvalidEnumTag) catch {

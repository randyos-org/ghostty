...
fn setoptTyped(
    ...
    switch (option) {
        .event => {
            if (comptime std.debug.runtime_safety) {
                _ = (std.enums.fromInt(TrackingMode, @intFromEnum(value.*)) orelse error.InvalidEnumTag) catch {
        ...
        .format => {
            if (comptime std.debug.runtime_safety) {
                _ = (std.enums.fromInt(Format, @intFromEnum(value.*)) orelse error.InvalidEnumTag) catch {
